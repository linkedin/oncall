...
class Scheduler(object):
    ...
    def create_events(self, team_id, schedule_id, user_id, events, role_id, cursor, skip_match=True, table_name='event'):
        ...
        logger.info("Step 2: Variables fetched")
        cursor.execute('''SELECT `name` FROM `roster`
                           WHERE `id` = %s''',
                       roster_id)
        roster_name = {row['name'] for row in cursor}
        ...
        for i in roster_name:
            roster_name_get=i
        a = data_fetch[0][1].get(str(roster_name_get))
        names = []
        ids = []
        for i in a:
            name = i.keys()[0]
        ...
        new_ids=[]
        logger.info("Step 4: ")
        while(len(new_names)<30):
            for i in range(0,len(names)):
                new_names.append(names[i])
                new_ids.append(ids[i])

        for i in range(0,len(new_names)):
            execute_query(t_id, s_id, start, end, new_ids[i], r_id)
            start+=86400
            end+=86400
        logger.info("#########################################")
