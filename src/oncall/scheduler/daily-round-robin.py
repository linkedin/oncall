...
class Scheduler(object):
    ...
    def create_events(self, team_id, schedule_id, user_id, events, role_id, cursor, skip_match=True, table_name='event'):
        ...
        s_id=schedule_id
        start=event['start']
        end=event['start']+86400
        u_id=user_id
        r_id=role_id
        cursor.execute('''SELECT `name` FROM `team`
                           WHERE `id` = %s''',
                       t_id)
        team_name = {row['name'] for row in cursor}
        cursor.execute('''SELECT `roster_id` FROM `schedule`
                           WHERE `id` = %s''',
                       s_id)
        roster_id = {row['roster_id'] for row in cursor}

        cursor.execute('''SELECT `user_id` FROM `roster_user`
                           WHERE `roster_id` = %s''',
                       roster_id)
        user_ids = [r['user_id'] for r in cursor]
        cursor.execute('''SELECT `auto_populate_threshold` FROM `schedule`
                           WHERE `id` = %s''',
                       s_id)
        threshold_days=[row['auto_populate_threshold'] for row in cursor]

        cursor.execute('''SELECT `last_scheduled_user_id` FROM `schedule`
                           WHERE `id` = %s''',
                       s_id)
    
        last_scheduled_user = [row['last_scheduled_user_id'] for row in cursor]
        if(not last_scheduled_user[0]):
            last_scheduled_user=[user_ids[len(user_ids)-1]]
    
        def execute_query(t_id, s_id, start, end, u_id, r_id):
            event_args = (t_id, s_id, start, end, u_id, r_id)
        ...
        last_index=user_ids.index(last_scheduled_user[0])
        if(last_index==len(user_ids)-1):
            user_index=0
        else:
            user_index=last_index+1
        
        new_user_ids=[]
        while(len(new_user_ids)<threshold_days[0]):
            new_user_ids.append(user_ids[user_index])
            if(user_index==len(user_ids)-1):
                user_index=0
            else:
                user_index+=1
        
        for i in range(0,len(new_user_ids)):
            execute_query(t_id, s_id, start, end, new_user_ids[i], r_id)
            start+=86400
            end+=86400
        cursor.execute('UPDATE `schedule` SET `last_scheduled_user_id` = %s WHERE `id` = %s', (new_user_ids[len(new_user_ids)-1], schedule_id))
    ...
    def populate(self, schedule, start_time, dbinfo, table_name='event'):
        ...
        #user_id = self.find_next_user_id(schedule, start_time, cursor, table_name)
        #self.create_events(team_id, schedule['id'], user_id, start_time, role_id, cursor, table_name=table_name)
